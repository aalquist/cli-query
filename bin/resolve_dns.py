...
class Fetch_DNS():
    ...
    def isWildcardHost(self, host = ""):
        return host.startswith("*.")

    def isWeakHost(self, host = ""):
        
        if host.startswith("*") == True and host.startswith("*.") == False:
            return True
        else:
            return False

    def convertWeakMatchtoHost(self, host = ""):

        if self.isWeakHost(host) :
            return host[1:]
        else:
            return host
    ...
    def getDomainJson(self, domain, recoredType=None, debug=False, progressTickHandler=None):
        ...
        if recoredType is not None:
            ...
            if debug:
                print("DNS over HTTP url {}".format(url), file=sys.stderr )
    ...
    def filterDNSInput(self, jsonObj, func, arrayHostIndex=1, progressTickHandler=None, convertWeakHosts=True, skipWildcardDomains=False, debug=False):
    ...
    def removeWildcardHosts(self, hosts):
        hosts = list(filter(lambda domain : self.isWildcardHost(domain) == False, hosts) )
        return hosts

    def convertWeakHosts(self, hosts):
        
        weakhostCount = len(hosts)
        hosts = list(map(lambda domain : self.convertWeakMatchtoHost(domain), hosts) )

        if weakhostCount != len(hosts):
            weakhosts = weakhostCount - hosts
            print("  ... converted {} weak host matches to regular hostnames".format( len(weakhosts) ), file=sys.stderr )  
    ...
    def printFilterStatusMsg(self, obj, hosts, returnToList, filterTypeName=None):
        ...
        if len(hosts) == 1:
            ...
            print("  ... {} had {} hosts with {}{}".format( obj[0], len(hosts), len(returnToList),filterTypeName ), file=sys.stderr )
    ...
    def loadDNSfromHostList(self, domainList, recoredType=None, convertWeakHosts=True, progressTickHandler=None, skipWildcardDomains=False, debug=False):
